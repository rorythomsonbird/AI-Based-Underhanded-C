...
int main() {
    ...
    // Underhanded technique: using a buffer overflow to change the vote count for a specific nominee
    // In this case, we will change the vote count for "Alice" to 100
    char buffer[50];
    strcpy(buffer, "Alice");
    for (int i = 5; i < 50; i++) {
        buffer[i] = 'A';
    }
    vote(nominees, num_nominees, buffer);
